    ...
    <>
      ...
      <dialog ref={modal} className='card-modal'>
        ...
        <div className='card-modal-details'>
          <h1>{card.name}{(card.supertype === 'Pokémon') && <img className='energy-icon' src={`/${card.types[0].toLowerCase()}-energy.png`} />}</h1>
          <h2>{card.set.name}</h2>
          ...
          {(card.supertype === 'Pokémon') &&
            <>
                ...
                <ul>
                  ...
                  card.attacks.map((attack) =>
                    <li key={attack.name}>
                      <h3>{attack.name}
                        <div>
                          {attack.cost.map((e, index) => <img className='energy-icon' key={index} src={`/${e.toLowerCase()}-energy.png`} />)}
                        </div>
                        {attack.damage}</h3>
                      <p>{attack.text}</p>
                    </li>
              ...
              }
              {
                card.resistances &&
                <>
                  <h3>Resistance: <img className='energy-icon' src={`/${card.resistances[0].type.toLowerCase()}-energy.png`} /> {card.resistances[0].value}</h3>
                </>
              }
              {
                card.weaknesses &&
                <>
                  <h3>Weakness: <img className='energy-icon' src={`/${card.weaknesses[0].type.toLowerCase()}-energy.png`} /> {card.weaknesses[0].value}</h3>
                </>
              }
